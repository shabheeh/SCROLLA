...
const EditArticle: React.FC<EditArticleProps> = ({ preferences, article, onCancel }) => {
  ...
  const [isArticleSaving, setArticleSaving] = useState<boolean>(false)
  ...
  const handleSave = async (data: ArticleFormValues) => {
    try {
      ...
      let featureImageUrl = article.featureImage;

      if (featureImage && article.featureImage !== imagePreview) {
      ...
      const editPromise = editArticle(article._id, articleData);
    
      toast.promise(editPromise, {
        loading: "Saving...",
        success: "Article saved",
        error: (err) => `Error: ${err instanceof Error ? err.message : "Something went wrong"}`,
      });
      ...
      <div className="md:col-span-7 space-y-6">
        {!imagePreview ? (
          ...
          <div className="relative rounded-lg overflow-hidden">
            ...
            />
            <div className="absolute bg-black rounded-full top-4 right-4 flex gap-2">
              <Button
                variant="secondary"
                size="icon"
                className="rounded-full text-white bg-background/80 backdrop-blur-md"
                onClick={removeImage}
              >
                <X className="h-4 w-4" />
              </Button>
        ...
        <div className="bg-muted/30 p-6 rounded-lg">
          ...
          {preferences.length > 0 && (
            <Tabs defaultValue={preferences[0]._id}>
              ...
              {preferences.map((preference) => (
                <TabsContent className="mt-7" key={preference._id} value={preference._id}>
        ...
        <div className="sticky top-24 space-y-6">
          ...
          onClick={() => onCancel()}
            ...
            disabled={isArticleSaving}
              onClick={articleForm.handleSubmit(handleSave)}
